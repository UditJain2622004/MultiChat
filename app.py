...
class ChatBot:
    def __init__(self, name, personality, description, image_filename):
        ...
        self.model = genai.GenerativeModel(
            ...
            Your goal is to embody this character and engage in a natural conversation. To do this effectively:
            ...
            Your response should be in this format: <name>@#$%: <Your response>'''
        )
    ...
    def respond(self):
        ...
        if len(response.text.split("@#$%:")) > 1:
            result = response.text.split("@#$%:")[1].strip()
        else:
            result = response.text
        central_history.append({"role": "user", "parts": [f'{self.name}@#$%: {result}']})
        return result
...
def send_message():
    ...
    central_history.append({"role": "user", "parts": [f'User@#$%: {message}']})
